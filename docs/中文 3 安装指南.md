...
把同一目录下的 **UpdateGeoPython.sh** 这个文件拖到终端（terminal）里面运行，就可以把 GeoPyTool.app 里面内置的 GeoPyTool 升级到最新版本了。千万别乱动这个文件同一目录下的任何其他内容，除非你很有信心能搞定哈，那样随你咯。
...
刚才上面那张图实际上是在 Windows7 系统上面运行 GeoPyTool 的截图。不过 Windows7 系统用户需要先安装几个补丁，才能运行 GeoPython。
...
然而并不行。你需要安装一些基础包，GeoPython 就是基于这些包构建的。即便你用的是 Anaconda3，也还是需要安装一些包，比如 pyqt5，这是用来搭建 GeoPyTool 的 GUI 图形界面的模块。
