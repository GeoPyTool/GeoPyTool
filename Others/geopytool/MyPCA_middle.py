...
class MyPCA(AppForm):
    ...
    def create_main_frame(self):
        ...
        self.legend_cb.stateChanged.connect(self.showLegend)  # int
        ...
        self.lda_cb.stateChanged.connect(self.showLDAboundaries)  # int

        self.hyperplane_cb= QCheckBox('&SVM')
        self.hyperplane_cb.setChecked(False)
        self.hyperplane_cb.stateChanged.connect(self.showSVMboundaries)  # int
        ...
        if(len(self.data_to_test)>0):
            ...
            if contained == True:
                ...
                for i in range(len(self.data_to_test)):
                    ...
                    if target not in test_labels and target not in self.whole_labels:
    ...
    def showPredictResult(self):
        ...
        try:
                ...
                Z = self.clf.predict(np.c_[self.pca_data_to_test])

                Z2 = self.clf.predict_proba(np.c_[self.pca_data_to_test])
