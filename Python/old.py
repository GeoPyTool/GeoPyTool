...
# Function: OLD Version of GeoPython
...
def plotpoint(x, y, size, color, alpha, marker='d'):
    # this function is used to put data point on the picture
    plt.scatter(x, y, marker=marker, s=size, color=color, alpha=alpha)
...
def tas(name="tas.xlsx", width=1, color='k'):
    ...
    if("csv"in name):tasraw  = pd.read_csv(name)
    elif("xlsx"in name):tasraw = pd.read_excel(name)
...
def qfl(name="qfl.xlsx", width=1, color='k'):
    ...
    if("csv" in name):qflraw = pd.read_csv(name)
    elif("xlsx" in name):qflraw = pd.read_excel(name)
...
def ree(name="ree.xlsx", Width=1, Color='b', Style="-"):
    ...
    if("csv"in name):reeraw = pd.read_csv(name)
    elif("xlsx"in name):reeraw = pd.read_excel(name)
        ...
        if (reeraw.at[i, 'DataType'] == 'User' or reeraw.at[i, 'DataType'] == 'user' or reeraw.at[
            ...
            for j in range(len(Element) - 1):
                ...
                plotpoint(j + 1, math.log((reeraw.at[i, Element[j]]) / reeraw.at[k, Element[j]]),
                          size=reeraw.at[i, 'Size'], color=reeraw.at[i, 'Color'], alpha=reeraw.at[i, 'Alpha'],
...
def qapf(name="qapf.xlsx", Width=1, Color='k'):
    ...
    if("csv"in name):qapfraw = pd.read_csv(name)
    elif("xlsx"in name):qapfraw = pd.read_excel(name)
...
def tribaseline(LineWidth=1, LineColor='k', l=u'left', r=u'right', u=u'up'):
...
def triseparate(x=[[0, 1, 2], [1, 1, 2], [1, 2, 3]]):
    a = []
    b = []
    c = []
    for i in x:
...
def triplot(x=1, y=1,z=1, size=1, color='r', alpha=0.5, marker='o'):
    #tribaseline
    tmp=tritrans(x,y,z)
    plotpoint(tmp[0],tmp[1], size=size, color=color, alpha=alpha, marker=marker)


def tridrawline(x=[0, 10], y=[0, 1], z=[0, 1], LineWidth=1, LineColor='k', LineStyle="-", LineAlpha=0.3, LineLabel=''):
    # tribaseline
    a = []
    b = []
    ...
    if (len(x) == len(y) == len(z)):
        for i in range(len(x)):
            tmp = tritrans(x[i], y[i], z[i])
            a.append(tmp[0])
            b.append(tmp[1])

    drawline(a, b, LineWidth=LineWidth, LineColor=LineColor, LineStyle=LineStyle, LineAlpha=LineAlpha,
             LineLabel=LineLabel)


def fp(name="fp.xlsx", width=1, color='k'):
    # Read the Excel, then draw the fpline, then Plot points, job done~

    tribaseline(LineWidth=1, LineColor='b', l=u'Ab', r=u'Or', u=u'An')

    if ("csv" in name):
        fpraw = pd.read_csv(name)
    elif ("xlsx" in name):
        fpraw = pd.read_excel(name)

    #        plotpoint((fpraw.at[i, 'Ab']), (fpraw.at[i, 'Or'] + fpraw.at[i, 'An']), fpraw.at[i, 'Size'],
    #                   fpraw.at[i, 'Color'], fpraw.at[i, 'Alpha'], fpraw.at[i, 'Marker'])
    ...
    for i in range(Points):
        ...
        triplot(fpraw.at[i, 'Ab'], fpraw.at[i, 'Or'], fpraw.at[i, 'An'], size=fpraw.at[i, 'Size'],
                color=fpraw.at[i, 'Color'], alpha=fpraw.at[i, 'Alpha'], marker=fpraw.at[i, 'Marker'])
        # size=1, color='r', alpha=0.5, marker='o')
        ...
        if (fpraw.at[i, 'Line'] == 1):
            wholeline.append([fpraw.at[i, 'An'], fpraw.at[i, 'Or'], fpraw.at[i, 'Ab']])
    ...
    plt.savefig("fp-Plot.png", dpi=600)
    plt.savefig("fp-Plot.svg", dpi=600)
    plt.show()


def test():
    print('It works now, you can use it~')
