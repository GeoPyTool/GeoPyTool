...
class XYZ(AppForm):
    ...
    def Magic(self):
        ...
        if (self.lda_cb.isChecked()):
            ...
            # This is to fit the LDA on the V-W data
            model.fit(LDA_X, original_label)

            # getting the range of V and W, this part is wrong due to the min and max of V and W do not correspond to the real Triangular area.
            #vmin,vmax=min(VtoFit),max(VtoFit)
            #wmin,wmax=min(WtoFit),max(WtoFit)

            xmin, xmax = 0.001,0.999
            ymin, ymax = 0.001,np.sqrt(3)/2-0.001

            a1,b1,c1 = self.BinToTri(xmin, ymin)
            a2,b2,c2 = self.BinToTri(xmax, ymax)

            print(a1,b1,c1 )
            print(a2,b2,c2 )

            vmin,wmin = self.LogRatioTriToBin(a1,b1,c1)
            vmax,wmax = self.DebugLogRatioTriToBin(a2,b2,c2)

            print(vmin,wmin,'\t',vmax,wmax)

            # building a meshgrid with vv and ww to do the prediction on chosen vv*ww area.
            vv, ww = np.meshgrid(np.linspace(vmin, vmax, 200),
                                 np.linspace(wmin, wmax, 200))

            # Z is the predict result for all data covered in the vv*ww area.
            Z = model.predict(np.c_[vv.ravel(), ww.ravel()])

            # get the prediction area on plots, wrong , still
            xplot,yplot = np.meshgrid(np.linspace(xmin, xmax, 200),
                                 np.linspace(ymin, ymax, 200))
            # generate the result, which does not fully cover the whole triangular at all.
            self.axes.contourf(xplot,yplot , Z.reshape(vv.shape), cmap=ListedColormap(self.color_list), alpha=0.2)
