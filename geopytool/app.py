...
class TASWindow(BasePlotWindow):
    def __init__(self, df=pd.DataFrame(), parent=None):
        ...
        self.cb_labels = QCheckBox("Show Labels"); self.cb_labels.setChecked(True); self.cb_labels.stateChanged.connect(self.plot)
        ...
        self.control_layout.addWidget(self.cb_labels); self.control_layout.addWidget(self.cb_regions); self.control_layout.addWidget(self.cb_irvine)
                ...
                if self.cb_labels.isChecked():
                    for i, txt in enumerate(df['Label']):
                        if pd.notna(txt) and txt != '': 
                            self.axes.annotate(str(txt), (df[sio2_col].iloc[i], df['__sum_alkali__'].iloc[i]), 
...
class REEWindow(BasePlotWindow):
    def __init__(self, df=pd.DataFrame(), parent=None):
        ...
        self.standards = {
            'Chondrite (Sun & McDonough 1989)': {'La': 0.237, 'Ce': 0.612, 'Pr': 0.095, 'Nd': 0.467, 'Sm': 0.153, 'Eu': 0.058, 'Gd': 0.2055, 'Tb': 0.0374, 'Dy': 0.254, 'Ho': 0.0566, 'Er': 0.1655, 'Tm': 0.0255, 'Yb': 0.17, 'Lu': 0.0254},
            'Chondrite (Taylor & McLennan 1985)': {'La': 0.367, 'Ce': 0.957, 'Pr': 0.137, 'Nd': 0.711, 'Sm': 0.231, 'Eu': 0.087, 'Gd': 0.306, 'Tb': 0.058, 'Dy': 0.381, 'Ho': 0.0851, 'Er': 0.249, 'Tm': 0.0356, 'Yb': 0.248, 'Lu': 0.0381},
            'MORB (Sun & McDonough 1989)': {'La': 2.5, 'Ce': 7.5, 'Pr': 1.32, 'Nd': 7.3, 'Sm': 2.63, 'Eu': 1.02, 'Gd': 3.68, 'Tb': 0.67, 'Dy': 4.55, 'Ho': 1.052, 'Er': 2.97, 'Tm': 0.46, 'Yb': 3.05, 'Lu': 0.46}
        }
...
class TraceWindow(BasePlotWindow):
    def __init__(self, df=pd.DataFrame(), parent=None):
        super().__init__(df, parent)
        self.axes = self.fig.add_subplot(111)
        self.elements = [u'Rb', u'Ba', u'Th', u'U', u'Nb', u'Ta', u'K', u'La', u'Ce', u'Pr', u'Sr', u'P', u'Nd', u'Zr', u'Hf', u'Sm', u'Eu', u'Ti', u'Tb', u'Dy', u'Y', u'Ho', u'Er', u'Tm', u'Yb', u'Lu']
        self.standards = {
            'PM (Sun & McDonough 1989)': {'Rb': 0.635, 'Ba': 6.989, 'Th': 0.085, 'U': 0.021, 'Nb': 0.713, 'Ta': 0.041, 'K': 250, 'La': 0.687, 'Ce': 1.775, 'Pr': 0.276, 'Sr': 21.1, 'P': 95, 'Nd': 1.354, 'Sm': 0.444, 'Zr': 11.2, 'Hf': 0.309, 'Eu': 0.168, 'Ti': 1300, 'Tb': 0.108, 'Dy': 0.736, 'Y': 4.55, 'Ho': 0.164, 'Er': 0.48, 'Tm': 0.074, 'Yb': 0.493, 'Lu': 0.074}
        }
        self.combo_standard = QComboBox(); self.combo_standard.addItems(self.standards.keys()); self.combo_standard.currentIndexChanged.connect(self.plot)
        self.control_layout.addWidget(QLabel("Normalization:")); self.control_layout.addWidget(self.combo_standard); self.control_layout.addStretch()
