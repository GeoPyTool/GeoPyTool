...
class MySVM(AppForm):
    ...
    def __init__(self, parent=None, df=pd.DataFrame()):
        ...
        self.setWindowTitle('SVM Data')

        self.items = []
        self._df = df
        if (len(df) > 0):
            self._changed = True
            # print('DataFrame recieved to SVM')
    ...
    def create_main_frame(self):
        ...
        self.dpi = 128
        self.setWindowTitle('SVM Figure')
        ...
        self.ShowValue_cb.stateChanged.connect(self.SVM)  # int

        self.ShowCorr_cb = QCheckBox('&Show Correlation Matrix on Plot')
        self.ShowCorr_cb.setChecked(False)
        self.ShowCorr_cb.stateChanged.connect(self.SVM)  # int
    ...
    def SVM(self):
        ...
        if able == False:
            reply = QMessageBox.information(self,'Warning','Your Data Failed to SVM.\n Please remove Non Valued items and Blanks!')
